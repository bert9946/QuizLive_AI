# windowcapture.py
# from https://stackoverflow.com/a/75629598/20062814
...
class WindowCapture:
    ...
    def __init__(self, given_window_name=None):
        if given_window_name is not None:
            ...
            if self.window is None:
                raise Exception(
                    'Unable to find window: {}'.format(given_window_name))
    ...
    def get_image_from_window(self):
        core_graphics_image = QZ.CGWindowListCreateImage(
            ...
            self.window_id,
            QZ.kCGWindowImageBoundsIgnoreFraming | QZ.kCGWindowImageBestResolution
        )
        ...
        core_graphics_data_provider = QZ.CGImageGetDataProvider(
            core_graphics_image)
        core_graphics_data = QZ.CGDataProviderCopyData(
            core_graphics_data_provider)
